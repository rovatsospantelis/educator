...
// One-pager → ένα SEO title/description για όλη τη σελίδα.
usePageSeo({
...
<template>
  <div>
    ...
    <section id="top" class="border-b border-line">
      <div class="mx-auto grid max-w-6xl items-center gap-12 px-6 pt-12 pb-16 md:grid-cols-2 md:py-28">
    ...
    <div id="about" class="scroll-mt-24 md:scroll-mt-28">
      <BaseSection eyebrow="Η καθηγήτρια" title="Ποια είμαι">
        <div class="grid items-start gap-10 md:grid-cols-[260px_1fr] md:gap-14">
          <div v-reveal class="mx-auto w-full max-w-[260px]">
            <!-- TODO: φωτογραφία -->
            <div class="aspect-[3/4] w-full rounded-2xl bg-bg-soft"></div>
            <p class="mt-3 text-center text-sm text-ink-soft md:text-left">{{ c.about.role }}</p>
          </div>
          <div v-reveal="{ delay: 120 }">
            <div class="space-y-5 leading-relaxed text-ink-soft">
              <p v-for="(p, i) in c.about.paragraphs" :key="i">{{ p }}</p>
            </div>
            <dl class="mt-10 grid grid-cols-3 gap-4 border-t border-line pt-8">
              <div v-for="h in c.about.highlights" :key="h.label" class="text-center md:text-left">
      ...
      <BaseSection eyebrow="Μαθήματα" title="Τι διδάσκω">
        <div class="grid gap-6 md:grid-cols-4">
          <div
            ...
            :key="s.level"
            v-reveal="i"
            class="flex flex-col rounded-2xl border border-line p-7"
          >
            <h3 class="text-2xl">{{ s.level }}</h3>
            <p class="mt-1 text-sm italic text-ink-soft">{{ s.note }}</p>
            <ul class="mt-5 space-y-2.5">
              <li v-for="item in s.items" :key="item" class="flex items-start gap-2.5 text-ink-soft">
                <Check :size="18" :stroke-width="2" class="mt-0.5 shrink-0 text-accent-deep" />
                <span>{{ item }}</span>
              </li>
            </ul>
          </div>
    ...
    <div id="contact" class="scroll-mt-24 md:scroll-mt-28">
      <BaseSection eyebrow="Επικοινωνία" title="Ας ξεκινήσουμε">
        <div class="grid gap-10 lg:grid-cols-2">
          ...
          <div v-reveal>
            ...
            <ul class="mt-8 space-y-5">
              <li v-if="info.address.area" class="flex items-start gap-4">
                <MapPin :size="22" :stroke-width="1.6" class="mt-0.5 shrink-0 text-accent-deep" />
                <div>
              ...
              <li v-if="info.phone" class="flex items-start gap-4">
                <Phone :size="22" :stroke-width="1.6" class="mt-0.5 shrink-0 text-accent-deep" />
                <div>
                  <p class="font-medium">Τηλέφωνο</p>
                  <a :href="'tel:' + info.phoneRaw" class="text-ink-soft hover:text-ink">{{ info.phone }}</a>
                </div>
              </li>
              <li v-if="info.mobile" class="flex items-start gap-4">
                <Smartphone :size="22" :stroke-width="1.6" class="mt-0.5 shrink-0 text-accent-deep" />
                <div>
                  <p class="font-medium">Κινητό</p>
                  <a :href="'tel:' + info.mobileRaw" class="text-ink-soft hover:text-ink">{{ info.mobile }}</a>
                </div>
              </li>
              <li v-if="info.email" class="flex items-start gap-4">
                <Mail :size="22" :stroke-width="1.6" class="mt-0.5 shrink-0 text-accent-deep" />
                <div>
